...
def show_coeff_time_graph(inst_id, times, coeff_arr_1, coeff_arr_2, coeff_idx, label_1 = "Prediction", label_2 = "Ground Truth", use_relu=True):
    ...
    plt.title("Instance Id " + str(inst_id) + ", Index " + str(coeff_idx))

# Shows the coeff vs time graph for single coeff_array and multiple indices n coeff_idx_list
def show_coeff_graphs(inst_id, times, coeff_array, coeff_idx_list, use_relu = True):
    from matplotlib import pyplot as plt
    ...
    plt.title("Instance Id " + str(inst_id) + ", Index {}".format(coeff_idx_list))

# Show dual-kpi curves from Exalibur data given as a dataframe
def show_train_dual_kpi_curves(df, inst_id, time_list, dual_scale_factor, grid_size, kpi="Spend"):
    from matplotlib import pyplot as plt
    ...
    for time in time_list:
        scaled_dual_x, kpi_y, scale = gen_unif_grid_dual_kpi(df, inst_id, time, dual_scale_factor, grid_size, kpi=kpi)
    ...
    plt.ylabel(kpi)
    ax.legend(time_list)
    plt.title("Instance Id " + str(inst_id) + ", Times " + str(time_list))

# Shows prior and posterior dual kpi curves and update points
def show_prior_post_dual_kpi_curves(model, likelihood, scale_df, inst_id, time_list, dual_scale_factor, grid_size, basis_funcs, kpi="Spend", use_relu=True):
    from matplotlib import pyplot as plt

    fig, ax = plt.subplots()

    colors = ["red", "blue", "green", "black", "brown", "pink"]
    scale = get_scale(scale_df, inst_id, dual_scale_factor)
    legend = []
    ...
    # Plotting new data
    plt.title("Instance Id " + str(inst_id) + ", Time " + str(time_list))